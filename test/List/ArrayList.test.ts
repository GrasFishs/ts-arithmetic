import { ArrayList } from '../../src/ds/List/Linear/ArrayList';

describe('ArrayList test:', () => {
  const arr = [1, 2, 3, 4, 5];
  test('init ArrayList when arg is empty.', () => {
    let list = new ArrayList<number>();
    expect(list.length).toEqual(0);
    expect(list.toArray()).toEqual([]);
  });

  test('init ArrayList from array', () => {
    let list = ArrayList.from(arr);
  ...
  test('insert before', () => {
    ...
    let list = ArrayList.from(innerArr);
